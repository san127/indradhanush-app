...
class SupabaseService {
  ...
  static Future<List<Map<String, dynamic>>> getMonthlyIncome() async {
    ...
    final response = await _client
        ...
        .gte(
          'evnt_date',
          sixMonthsAgo.toString().split(' ')[0],
        ) //del the tiem part of the date string
  ...
  static Future<List<Map<String, dynamic>>> checkEventsForDate(
    DateTime date,
  ) async {
  ...
  static Stream<List<Map<String, dynamic>>> eventStream() {
    return _client.from('events').stream(primaryKey: ['evnt_id']);
  }
  ...
  static Future<int> getEstimatedRevenueForMonth(DateTime month) async {
    final start = DateTime(month.year, month.month, 1);
    ...
    final response = await _client
        .from('events')
        .select('amount')
        .gte('evnt_date', start.toIso8601String().split('T')[0])
        .lt('evnt_date', end.toIso8601String().split('T')[0]);
  ...
  static Future<List<Map<String, dynamic>>> getAdvancesForMonth(
    DateTime month,
  ) async {
    final start = DateTime(month.year, month.month, 1);
    ...
    final events = await _client
        .from('events')
        .select('evnt_id')
        .gte('evnt_date', DateFormat('yyyy-MM-dd').format(start))
        .lt('evnt_date', DateFormat('yyyy-MM-dd').format(end));

    if (events.isEmpty) {
      return [];
    }

    final eventIds = events.map((e) => e['evnt_id']).toList();

    final advances = await _client
        .from('advanceDetails')
        .select('adv_amount, adv_mode, adv_paid_date, event_id')
        .inFilter('event_id', eventIds);

    print('advances = $advances');

    return List<Map<String, dynamic>>.from(advances);
  }
  ...
  static Future<List<Map<String, dynamic>>> getPaymentsForMonth(
    DateTime month,
  ) async {
    final start = DateTime(month.year, month.month, 1);
    ...
    final payments = await _client
        .from('payments')
        .select('final_amount_paid, pay_mode, pay_date, event_id')
        .inFilter('event_id', eventIds);
  ...
  static Future<int> getActualRevenueForMonth(DateTime month) async {
    final advances = await getAdvancesForMonth(month);
    ...
    for (final row in advances) {
      total += ((row['adv_amount'] ?? 0) as num).toInt();
    }

    for (final row in payments) {
      total += ((row['final_amount_paid'] ?? 0) as num).toInt();
    }
  ...
  static Future<Map<String, int>> getRevenueBreakdownForMonth(
    DateTime month,
  ) async {
    final advances = await getAdvancesForMonth(month);
    ...
    void addAmount(String? mode, int amount) {
    ...
    for (final row in payments) {
      addAmount(
        ...
        ((row['final_amount_paid'] ?? 0) as num).toInt(),
      );
    }
  ...
  static Future<void> deleteExpense(int expenseId) async {
    await _client.from('Expenses').delete().eq('exp_id', expenseId);
  }
  ...
  static Future<void> updateExpense({
    ...
    await _client
        ...
        .update({
          ...
          'exp_date': DateFormat('yyyy-MM-dd').format(date),
        })
        .eq('exp_id', expenseId);
  }

  //cash memo method
  static Future<int> createCashMemo(Map<String, dynamic> memo) async {
  ...
  static Future<void> addCashMemoItems(List<Map<String, dynamic>> items) async {
    await _client.from('cash_memo_items').insert(items);
  }

  //fetch event specific cash memo
  static Future<List<Map<String, dynamic>>> getCashMemosForEvent(
    String eventId,
  ) async {
    final response = await _client
        .from('cash_memos')
  ...
  static Future<String> generateMemoNumber(String eventId) async {
    final events = await _client
        .from('events')
    ...
    final event = eventList.firstWhere((e) => e['evnt_id'] == eventId);

    final date = DateTime.parse(event['evnt_date']);

    final monthlyEvents = eventList.where((e) {
    ...
    final eventNumber =
        monthlyEvents.indexWhere((e) => e['evnt_id'] == eventId) + 1;
  ...
  static Future<Map<String, dynamic>?> getCashMemoForEvent(
    String eventId,
  ) async {
    final response = await _client
        .from('cash_memos')
  ...
  static Future<List<Map<String, dynamic>>> getCashMemoItems(int memoId) async {
    final response = await _client
  ...
  static Future<void> updateCashMemo(
    int memoId,
    Map<String, dynamic> data,
  ) async {
    await _client.from('cash_memos').update(data).eq('memo_id', memoId);
  }

  static Future<void> deleteCashMemoItems(int memoId) async {
    await _client.from('cash_memo_items').delete().eq('memo_id', memoId);
  }
}
